(() => {
  ...
  function afficherValidation(data) {
    ...
    titreResume.textContent = "Résumé de l’ajustement COACH";
    ...
    if (lignesResume.length > 0) {
      ...
      texte.textContent =
        "Aucun résumé de l’ajustement COACH n’a été produit.";
    ...
    note.textContent = listeAlertes.length > 0
      ...
      : "L’ajustement COACH est prêt à être validé.";
  ...
  async function initialiserPage() {
    ...
    if (!boutonValider) {
      throw new Error("Bouton de validation COACH absent.");
    }
