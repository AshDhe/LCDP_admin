...
(() => {
  ...
  const PAGE = {
  "title": "AjustCoach",
  "prefix": "lcdp-ajust-coach",
  "identiteId": "lcdp-ajust-coach-identite",
  "briefRoute": "/coach/ajust/brief",
  "validateRoute": "/coach/ajust/valider",
  "resumeTitle": "Résumé de l’ajustement COACH",
  "readyMessage": "L’ajustement COACH est prêt à être validé.",
  "confirmTitle": "Validation AjustCoach",
  "confirmMessage": "Valider cet ajustement COACH et écrire les états correspondants ?",
  "analysisMessage": "Analyse IA de l’ajustement COACH en cours…",
  "analysisReadyMessage": "Ajustement IA COACH prêt à être vérifié.",
  "validationMessage": "Écriture de l’ajustement COACH en cours. Ne ferme pas cette page.",
  "successMessage": "Ajustement COACH enregistré.",
  "focusId": "lcdp-ajust-coach-date-debut"
};
  ...
  function endpointWorker() {
    return String(
      config.workerAjustCoachAdminUrl ||
      config.WORKER_AJUST_COACH_ADMIN_URL ||
      window.ADMIN_CONFIG?.API_AJUST_COACH_ADMIN ||
      ""
    ).replace(/\/+$/, "");
  }
  ...
  async function chargerParc() {
    ...
    if (!endpoint) {
      throw new Error("Endpoint AjustCoach non configuré.");
    }
...
function joursSelectionnes() {
  return Array.from(
    document.querySelectorAll(
      'input[name="lcdp-ajust-coach-jour"]:checked'
    )
  ).map((input) => input.value);
}

function actualiserCapacite() {
  const action = valeur("lcdp-ajust-coach-action");
  const zone = document.querySelector("[data-capacite-zone]");
  const input = document.getElementById("lcdp-ajust-coach-capacite");
...
function construireBrief() {
  const action = valeur("lcdp-ajust-coach-action");
  const dateDebut = valeur("lcdp-ajust-coach-date-debut");
  const dateFin = valeur("lcdp-ajust-coach-date-fin");
  const debut = valeur("lcdp-ajust-coach-debut");
  const fin = valeur("lcdp-ajust-coach-fin");
  const jours = joursSelectionnes();
  const precision = valeur("lcdp-ajust-coach-precision");
  const capaciteTexte = valeur("lcdp-ajust-coach-capacite");
  const capaciteNombre = capaciteTexte
    ? Number.parseInt(capaciteTexte, 10)
    : null;

  if (!["ouvrir", "fermer"].includes(action)) {
    throw new Error("L’action COACH est invalide.");
  }

  if (!dateDebut || !dateFin || dateDebut > dateFin) {
    throw new Error("La période de l’ajustement COACH est invalide.");
  }
  ...
  if (!debut || !fin) {
    throw new Error("La plage horaire COACH est incomplète.");
  }
  ...
  if (
    ...
    throw new Error("La capacité COACH est obligatoire pour un ajustement exclusif.");
  }

  const joursTexte = jours.join(", ");
  const ajustement = action === "ouvrir"
    ? (
        "AJUST COACH : réserver exclusivement COACH du " +
        dateDebut + " au " + dateFin + ", les " + joursTexte +
        ", de " + formaterHeure(debut) + " à " +
        formaterHeure(fin) + ". Écrire COACH à true et " +
        "DUO et FAMILLE à false ou null selon leur état."
      )
    : (
        "FERMETURE AJUST COACH : annuler l’ajustement exclusif du " +
        dateDebut + " au " + dateFin + ", les " + joursTexte +
        ", de " + formaterHeure(debut) + " à " +
        formaterHeure(fin) + ". Écrire COACH à false et restaurer " +
...
function initialiserComportementsFormulaire() {
  document.getElementById("lcdp-ajust-coach-action")
  ...
  async function analyser(event) {
    ...
    if (!endpoint) {
      afficherStatus("Endpoint AjustCoach non configuré.", true);
      return;
    }
