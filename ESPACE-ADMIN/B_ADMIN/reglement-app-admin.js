(() => {
  ...
  const CHEMIN_ROUTE =
    "/ESPACE-ADMIN/B_ADMIN/reglement-app-admin.html";
  ...
  async function chargerDocument() {
    return appelerJson("/admin/reglement-app");
  }

  async function initialiserEditeur(data) {
    const slot = document.querySelector(
      "[data-lcdp-reglement-app-editeur-slot]"
    );
    ...
    const blocsInitiaux =
      Array.isArray(data?.brouillon?.blocs) &&
      data.brouillon.blocs.length > 0
        ? data.brouillon.blocs
        : [
            {
              id: "bloc-1",
              titre: "",
  ...
  function afficherStatus(message, etat = "") {
    const status = document.querySelector(
      "[data-lcdp-reglement-app-status]"
    );
  ...
  function verrouillerActions(verrouiller) {
    ...
    document
      .querySelectorAll(
        "[data-lcdp-reglement-app-actions] button"
      )
      .forEach((bouton) => {
  ...
  async function publier() {
    ...
    try {
      ...
      modificationsLocales = false;
      actualiserMeta();
      afficherStatus("Règlement application publiées.");
      await afficherAlerte("Règlement application publiées.");
    } catch (error) {
      afficherStatus(
        String(error?.message || error || ""),
        "error"
  ...
  function previsualiser() {
    ...
    const preview = document.querySelector(
      "[data-lcdp-reglement-app-preview]"
    );
    const contenu = document.querySelector(
      "[data-lcdp-reglement-app-preview-content]"
    );
  ...
  async function demanderConfirmationPublication() {
    ...
    titre.textContent = "Publier les règlement application";
    texte.textContent =
  ...
  async function initialiserPage() {
    ...
    const loading = document.querySelector(
      "[data-lcdp-reglement-app-loading]"
    );
    const actions = document.querySelector(
      "[data-lcdp-reglement-app-actions]"
    );
    ...
    document
      .querySelector("[data-lcdp-reglement-app-previsualiser]")
      ?.addEventListener("click", previsualiser);

    document
      .querySelector("[data-lcdp-reglement-app-enregistrer]")
      ?.addEventListener("click", enregistrerBrouillon);

    document
      .querySelector("[data-lcdp-reglement-app-publier]")
