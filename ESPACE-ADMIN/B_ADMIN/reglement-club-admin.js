(() => {
  ...
  const CHEMIN_ROUTE =
    "/ESPACE-ADMIN/B_ADMIN/reglement-club-admin.html";
  ...
  async function chargerDocument() {
    return appelerJson("/admin/reglement-club");
  }

  async function initialiserEditeur(data) {
    const slot = document.querySelector(
      "[data-lcdp-reglement-club-editeur-slot]"
    );
  ...
  function afficherStatus(message, etat = "") {
    const status = document.querySelector(
      "[data-lcdp-reglement-club-status]"
    );
  ...
  function verrouillerActions(verrouiller) {
    ...
    document
      .querySelectorAll(
        "[data-lcdp-reglement-club-actions] button"
      )
      .forEach((bouton) => {
  ...
  async function enregistrerBrouillon() {
    ...
    try {
      const data = await appelerJson(
        "/admin/reglement-club/brouillon",
        {
          method: "POST",
          body: {
  ...
  async function publier() {
    ...
    try {
      ...
      modificationsLocales = false;
      actualiserMeta();
      afficherStatus("Règlement club publiées.");
      await afficherAlerte("Règlement club publiées.");
    } catch (error) {
      afficherStatus(
        String(error?.message || error || ""),
        "error"
  ...
  function previsualiser() {
    ...
    const preview = document.querySelector(
      "[data-lcdp-reglement-club-preview]"
    );
    const contenu = document.querySelector(
      "[data-lcdp-reglement-club-preview-content]"
    );
  ...
  async function demanderConfirmationPublication() {
    ...
    titre.textContent = "Publier les règlement club";
    texte.textContent =
  ...
  async function initialiserPage() {
    ...
    const loading = document.querySelector(
      "[data-lcdp-reglement-club-loading]"
    );
    const actions = document.querySelector(
      "[data-lcdp-reglement-club-actions]"
    );
    ...
    document
      .querySelector("[data-lcdp-reglement-club-previsualiser]")
      ?.addEventListener("click", previsualiser);

    document
      .querySelector("[data-lcdp-reglement-club-enregistrer]")
      ?.addEventListener("click", enregistrerBrouillon);

    document
      .querySelector("[data-lcdp-reglement-club-publier]")
